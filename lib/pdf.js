...
function prepareTemplateData(cvData) {
  ...
  if (d.skills) {
    ...
    // Boolean flags for template conditionals. Templates previously used
    // {{#skills.technical.length}} (3 dot-levels), but renderTemplate()'s
    // section-tag regexes only support up to 2 dot-levels (e.g. "skills.foo"),
    // so those tags silently failed to match and leaked into rendered PDFs as
    // literal text. Flat 2-level booleans like "skills.hasTechnical" match fine.
    d.skills.hasTechnical      = (d.skills.technical      || []).length > 0;
    d.skills.hasSoft            = (d.skills.soft            || []).length > 0;
    d.skills.hasLanguages       = (d.skills.languages       || []).length > 0;
    d.skills.hasCertifications  = (d.skills.certifications  || []).length > 0;
  }

  const ex = d.extras || {};
  d.extras = {
    ...ex,
    hasAwards:    (ex.awards    || []).length > 0,
    hasProjects:  (ex.projects  || []).length > 0,
    hasVolunteer: (ex.volunteer || []).length > 0,
  };
  d.hasExtras = d.extras.hasAwards || d.extras.hasProjects || d.extras.hasVolunteer;

  // resume-builder skill design system: keyword strip + competency grid.
  // Keyword strip — closest available proxy to the skill's "12-15 ATS keywords"
  // without a dedicated AI keyword-extraction pass: technical skills + certs.
  if (d.skills) {
    const pool = [...(d.skills.technical || []), ...(d.skills.certifications || [])];
    const keywords = [...new Set(pool)].slice(0, 10);
    d.skills.keywords_str = keywords.join(' &nbsp;|&nbsp; ');
    d.skills.hasKeywords = keywords.length > 0;

    // Core competencies grid — soft skills, falling back to technical.
    const competencies = (d.skills.soft && d.skills.soft.length ? d.skills.soft : d.skills.technical) || [];
    d.skills.competencies = competencies;
    d.skills.hasCompetencies = competencies.length > 0;
  }
