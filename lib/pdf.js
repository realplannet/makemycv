...
async function renderPDF(cvData, templateName) {
  ...
  // CRITICAL for Vercel: Chromium's bundled .so files (libnspr4, libnss3, etc.)
  // are extracted next to the executable in /tmp — but the dynamic linker won't
  // find them unless LD_LIBRARY_PATH points there. Without this, launch fails
  // with "error while loading shared libraries" on Vercel's runtime.
