 ...
 * Reuses the same Resend setup as functions/api/email.js (RESEND_API_KEY,
 * EMAIL_DOMAIN). Best-effort: if the alert itself fails to send, we log it
 * but never let that mask or replace the original generation error.
 */
import { Resend } from 'resend';

export async function sendGenerationFailureAlert({ sessionId, razorpayOrderId, razorpayPaymentId, contact, template, mode, error }, env) {
  ...
  try {
    ...
    const to = env.ADMIN_ALERT_EMAIL || 'hello@realplannet.com';

    await resend.emails.send({
      from: `MakeMyCV Alerts <noreply@${env.EMAIL_DOMAIN || 'realplannet.com'}>`,
      to,
