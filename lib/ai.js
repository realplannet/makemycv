...
export async function generateCVFromUpload({ uploadedFile, notes, guidedAdds, contact }, env) {
  ...
  if (uploadedFile && uploadedFile.data) {
    ...
    if (ext.includes('pdf')) {
      ...
      //
      // IMPORTANT: mammoth's package.json remaps lib/unzip.js -> browser/unzip.js
      // for bundlers (browser/package.json "browser" field). Cloudflare's esbuild
      // bundler resolves to that browser build, NOT the Node one, even with
      // nodejs_compat on. The browser build's openZip() only recognises
      // `{ arrayBuffer }` — passing `{ buffer }` (a Node Buffer, which is what the
      // Node-targeted docs/examples show) silently fails with "Could not find
      // file in options" because the browser build never even looks at `.buffer`.
      // Root-caused 2026-08-07 after a real paid customer hit this in production
      // (upload mode, .docx file) — reproduced under `wrangler pages dev` against
      // a real docx buffer before fixing, don't revert to `{ buffer }`.
      const binary = atob(uploadedFile.data);
